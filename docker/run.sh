...
docker run -it --rm --rm --net=host --ipc=host --pid=host --privileged -e RMW_IMPLEMENTATION=rmw_fastrtps_cpp --gpus=all -e NVIDIA_DRIVER_CAPABILITIES=all -e DISPLAY=$DISPLAY -v $(pwd):/ezbot-v2 -w /ezbot-v2 --name "ez-botV2" -e "TERM=xterm-256color" ezbot-v2-dev /bin/bash 
